...
Соответствие кода принятым стандартам оформления кода; 
Комментарии к классам, методам, сложным участкам кода;
...
Все динамические данные, помещаемые в SQL-запрос, обязательно должны проходить обработку: значения полей таблиц – экранироваться, сами имена полей или таблиц – проверяться по белому списку допустимых значений;
...
Статическая информация, имеющая отношение к проекту (пути к директориям, размеры изображений, e-mail'ы, ключи Google Maps и т.д.) должны выноситься в конфиг приложения;
...
Нельзя полагаться на то, что проект будет привязан к определенному домену, или даже будет находиться в корневой директории вебсервера;
...
Ссылки:
Best MVC Practices [версия на русском]
JavaScript
...
Общие ссылки:
PHP: The Right Way
Patterns, UML and Refactoring
The Definitive Guide to Yii [версия на русском]
...
Исключения:
Если фреймворк использует автозагрузчик классов несовместимый с PSR-0, можно использовать именование классов, наиболее удобное для использования в этом фреймворке.
...
Значения полей в обязательном порядке должны быть экранированы.
...
Имена таблиц и полей таблиц должны фильтроваться по белому списку.
...
Ссылки:
Habrahabr: Защита от SQL-инъекций в PHP и MySQL
PHP: PDO — Manual
OWASP: SQL Injection
Yii DAO: binding parameters
...
// неправильно:
`mysqldump --user=abc —-password=Wow!IHaveAPassword abc_db > /backup/backup.sql`;
...
`mysqldump --user=$dbUser —-password=$dbPassword $dbName > {$backupPath}backup.sql`;
...
// правильно:
const ERROR_LIMIT_CANNOT_BE_NEGATIVE = 'Число объектов не может быть отрицательным!'ж
. . .
    if ($limit < 0)
